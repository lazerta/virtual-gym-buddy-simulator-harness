...
def run_external_then_personal_validation(artifact_dir: str | Path = "artifacts/profile_ordered_validation", *, false_cue_n: int = 20_000, subject_lock_seeds: int = 500, false_cue_gate: FalseCueGate | None = None) -> dict:
    ...
    srd=run_rep_detection_benchmark(SHAWN_VARIANTS,base_seed=3760000); _save_df(srd,out/'08_shawn_rep_detection.csv')
    ok=bool(srd.passed.all()); stages.append(Stage('shawn_rep_detection','PASS' if ok else 'FAIL',ok,{"passed":int(srd.passed.sum()),"total":len(srd),"cases":int(srd.case.nunique())}))
    if not ok: return _finish(out,stages,personal_allowed=True)
    sd=h.deterministic(SHAWN_VARIANTS,calibrated=True); _save_df(sd,out/'09_shawn_base_deterministic.csv')
    ok=bool(sd.passed.all()); stages.append(Stage('shawn_base_deterministic','PASS' if ok else 'FAIL',ok,{"passed":int(sd.passed.sum()),"total":len(sd)}))
    if not ok: return _finish(out,stages,personal_allowed=True)
    sg=h.commercial_gym_deterministic(SHAWN_VARIANTS,calibrated=True); _save_df(sg,out/'10_shawn_commercial_gym.csv')
    ok=bool(sg.passed.all()); stages.append(Stage('shawn_commercial_gym','PASS' if ok else 'FAIL',ok,{"passed":int(sg.passed.sum()),"total":len(sg),"scenes":int(sg.gym_scene.nunique())}))
    if not ok: return _finish(out,stages,personal_allowed=True)
    sep=run_commercial_gym_episode_benchmark(SHAWN_VARIANTS,seeds_per_episode=1,base_seed=3920000,fps=10); _save_df(sep,out/'11_shawn_commercial_gym_episodes.csv')
    ok=bool(sep.passed.all()); stages.append(Stage('shawn_commercial_gym_episodes','PASS' if ok else 'FAIL',ok,{"passed":int(sep.passed.sum()),"total":len(sep),"episode_templates":int(sep.episode.nunique()),"wrong_identity_locks":int(sep.wrong_identity_locks.sum()),"false_reps":int(sep.false_reps.sum()),"unsafe_biomechanics_frames":int(sep.unsafe_biomechanics_frames.sum()),"max_reacquire_s":float(sep.max_reacquire_s.max())}))
    if not ok: return _finish(out,stages,personal_allowed=True)
    sfc=run_false_cue_benchmark(max(5000,false_cue_n//4),seed=830003,calibrated=True,mode='natural',subjects=SHAWN_VARIANTS); _save_df(sfc,out/'12_shawn_natural_false_cue.csv')
    sfcs=false_cue_summary(sfc,gate); sok=bool(sfcs['passed_gate']); stages.append(Stage('shawn_natural_false_cue','PASS' if sok else 'FAIL',sok,sfcs))
    scomp=h.composite_fuzz(5_000,seed=830004,calibrated=True,subjects=SHAWN_VARIANTS,max_components=3); _save_df(scomp,out/'13_shawn_composite_fuzz.csv')
    stages.append(Stage('shawn_composite_fuzz','DIAGNOSTIC',None,{"pass_rate":float(scomp.passed.mean()),"passed":int(scomp.passed.sum()),"total":len(scomp)}))
    return _finish(out,stages,personal_allowed=True)


def _finish(out: Path, stages: list[Stage], personal_allowed: bool) -> dict:
    data={"policy":"external profiles must pass all blocking gates before Shawn-like profiles run","personal_stage_allowed":personal_allowed,"stages":[asdict(x) for x in stages]}
...
def run_representative_then_personal_validation(artifact_dir: str | Path = "artifacts/representative_then_personal", *, fps: int = 10, representative_profile_id: str = "ansur_median_central") -> dict:
    ...
    if not ok: return _finish_quick(out,stages,personal_allowed=False,representative=ext.id)
    srd=run_rep_detection_benchmark([SHAWN_VARIANTS[0]],fps=20,base_seed=6010000); _save_df(srd,out/'03_shawn_rep_detection.csv')
    ok=bool(srd.passed.all()); stages.append(Stage('shawn_rep_detection','PASS' if ok else 'FAIL',ok,{'profile':SHAWN_VARIANTS[0].id,'passed':int(srd.passed.sum()),'total':len(srd),'cases':int(srd.case.nunique())}))
    if not ok: return _finish_quick(out,stages,personal_allowed=True,representative=ext.id)
    sep=run_commercial_gym_episode_benchmark([SHAWN_VARIANTS[0]],seeds_per_episode=1,base_seed=6510000,fps=fps); _save_df(sep,out/'04_shawn_commercial_gym_episodes.csv')
    ok=bool(sep.passed.all()); stages.append(Stage('shawn_commercial_gym_episodes','PASS' if ok else 'FAIL',ok,{'profile':SHAWN_VARIANTS[0].id,'passed':int(sep.passed.sum()),'total':len(sep),'episode_templates':int(sep.episode.nunique()),'wrong_identity_locks':int(sep.wrong_identity_locks.sum()),'false_reps':int(sep.false_reps.sum()),'unsafe_biomechanics_frames':int(sep.unsafe_biomechanics_frames.sum()),'max_reacquire_s':float(sep.max_reacquire_s.max())}))
...
def _finish_quick(out: Path, stages: list[Stage], personal_allowed: bool, representative: str) -> dict:
    data={'policy':'representative anonymous profile must pass blocking gates before Shawn profile runs','representative_profile':representative,'personal_stage_allowed':personal_allowed,'stages':[asdict(x) for x in stages]}
    (out/'summary.json').write_text(json.dumps(data,indent=2),encoding='utf-8')
    return data
