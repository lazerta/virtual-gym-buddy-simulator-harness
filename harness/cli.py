...
def main(argv=None):
    ...
    fc.add_argument("--calibrated",action="store_true")
    fc.add_argument("--mode",choices=["natural","boundary"],default="natural")
    ...
    val.add_argument("--quick",action="store_true",help="run one representative anonymous profile, then personal")
    ...
    if a.cmd=="rep-detection":
        ...
        if (~df.passed).any():
            print(df.loc[~df.passed].head(20).to_string(index=False))
